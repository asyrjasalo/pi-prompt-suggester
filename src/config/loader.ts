...
export class FileConfigLoader implements ConfigLoader {
	...
	public async load(): Promise<PromptSuggesterConfig> {
		...
		const globalPath = path.join(this.homeDir, ".pi", "agent", "extensions", "pi-prompt-suggester", "config.json");
		const projectPath = path.join(this.cwd, "etc", "pi", "extensions", "pi-prompt-suggester", "config.json");

		const defaultConfig = await readRequiredConfig(defaultPath);
		const globalConfig = await readOverrideConfig(globalPath, defaultConfig);
		const mergedGlobal = deepMerge(defaultConfig, globalConfig);
		const projectConfig = await readOverrideConfig(projectPath, defaultConfig);
		const merged = deepMerge(mergedGlobal, projectConfig);
		const normalized = normalizeConfig(merged, defaultConfig);

		if (!validateConfig(normalized.config)) {
			throw new Error(
				`Failed to normalize suggester config. Base defaults from ${defaultPath}; global from ${globalPath}; project from ${projectPath}.`,
			);
		}
		return normalized.config;
	}
